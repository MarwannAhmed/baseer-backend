...
from app.features.arabic_ocr.postprocess.reranker import RERANKER
from app.features.arabic_ocr.utils.arabic_utils import is_arabic_char
from app.features.arabic_ocr.config import OUTPUT_DIR


def count_non_arabic(text):
    return sum(1 for ch in text if not is_arabic_char(ch) and not ch.isspace())


def run_once(pipeline: ArabicOCRPipeline, image_path):

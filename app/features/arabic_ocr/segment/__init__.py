...
def segment(binary):
    ...
    for line_idx, (ly1, ly2, line_img) in enumerate(lines):
        ...
        for paw_idx, (px1, px2, paw_img) in enumerate(paws):
            ...
            #fall back to a single box covering the entire PAW so words are not dropped.
            if not char_boxes:
            ...
            for char_idx, (cx1, cy1, cx2, cy2) in enumerate(char_boxes):
                ...
                if dot_list:
                    ...
                    # builds list for this char index in the loop below by matching index
                else:
                    assigned = {}

                # fetches assigned dots for this char index if available
...
def _position_tag(char_idx, n_chars):
    if n_chars == 1:
        return "isolated"
    if char_idx == 0:
        return "initial"

...
infer_color.py
──────────────
Run color inference on one or more images using the trained color_svm.pkl.
No retraining. No ONNX. Just the raw pkl.

Usage:
  python infer_color.py --model results/color_svm.pkl image1.jpg image2.jpg
  python infer_color.py --model results/color_svm.pkl --folder ./test_images
"""
...
# ── Constants (must match train_and_eval.py) ──────────────────────────────────
PIXEL_STRIDE            = 3
...
def extract_features(bgr: np.ndarray):
    """
    Same pipeline as train_and_eval.py extract_dominant_lab() + make_feature().
    No AWB — raw LAB only.
    Returns (features_1x5, L, A, B, chroma, hue) for debugging.
    """
    ...
    km = KMeans(n_clusters=k, n_init=1, max_iter=KMEANS_MAX_ITER,
                random_state=0).fit(sat)

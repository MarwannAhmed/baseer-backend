...
class SVMClassifier:
    def __init__(self):
        with open(parameters["SVM"]["model_path"],   "rb") as f: self._clf     = pickle.load(f)
        with open(parameters["SVM"]["scaler_path"],  "rb") as f: self._scaler  = pickle.load(f)
        with open(parameters["SVM"]["encoder_path"], "rb") as f: self._encoder = pickle.load(f)

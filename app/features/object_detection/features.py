...
def sift_descriptor_feature(grayscale_patch):
    ...
    for row in range(grid_size):
        for column in range(grid_size):
            ...
            gauss = (
                cv2.getGaussianKernel(cell_height, cell_height / 2.0)
                * cv2.getGaussianKernel(cell_width, cell_width / 2.0).T
            )
...
def extract_features(image_patch):
    ...
    if len(resized_patch.shape) == 3 and resized_patch.shape[2] == 3:
        grayscale_patch = cv2.cvtColor(resized_patch, cv2.COLOR_BGR2GRAY).astype(
            np.float32
        )
    else:

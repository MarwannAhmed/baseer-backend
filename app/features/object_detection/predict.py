...
def detect_objects(image, confidence_threshold=0.85):
    ...
    print(
        f" Selective Search results: {len(raw)} raw proposals, reduced to "
        f"{len(region_proposals)} after filtering."
    )
    ...
    boxes, scores, labels = non_maximum_suppression(
        all_boxes, all_scores, all_labels, iou_threshold=0.15
    )
    if len(boxes) > 1:
        ...
        while len(ordered_scores) > 0:
            ...
            ious = np.array(
                [compute_iou(boxes_array[best], boxes_array[i]) for i in rest]
            )

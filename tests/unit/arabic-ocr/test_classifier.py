...
class TestCNNClassifier:
    def setup_method(self):
        ...
        for letter in classes:
            for _ in range(15):
                imgs.append(np.random.randint(0, 256, (32, 32), dtype=np.uint8))
                labels.append(letter)

        self.clf = CNNClassifier(epochs=2)
        self.clf.train(np.stack(imgs), np.array(labels))
        self.test_img = imgs[0]

    def test_predict_returns_candidates(self):
        result = self.clf.predict(self.test_img)
        assert 0 < len(result) <= TOP_K

    def test_predict_chars_are_arabic(self):
        for char, conf in self.clf.predict(self.test_img):
            assert is_arabic_char(char), f"Not Arabic: {char!r}"

    def test_predict_confidences_descending(self):
        confs = [c for _, c in self.clf.predict(self.test_img)]
        assert confs == sorted(confs, reverse=True)

    def test_predict_batch_length(self):
        results = self.clf.predict_batch([self.test_img] * 3)
        assert len(results) == 3
        for r in results:
            assert 0 < len(r) <= TOP_K
